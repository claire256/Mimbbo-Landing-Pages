...
const faqs = [
            ...
            { q: 'Is it safe to do a house calls?', 
              a: (
                <>
                Every client who requests a Mimbbo at Home service must complete our verification process before their job goes live. Here’s how we protect you:
                <ul className="list-disc list-inside mt-2 space-y-1">
          <li>ID Verification: Clients are required to upload a government-issued ID for identity confirmation.</li>
          <li>Verified Contact Info: We check email and phone numbers before they can post gigs..</li>
          <li>Location Transparency: You'll know the type of location (home, hotel, dorm, etc.) before accepting any job.</li>
        </ul>
              </>
              ),
                },
      ...
      <div className="space-y-4">
        {faqs.map((faq, index) => (
          <div key={index} className="border-b border-gray-300 pb-3">
            ...
            {openIndex === index && (
              <div className="text-dark text-[15px] mt-3">{faq.a}</div>
            )}
          </div>
        ))}
      </div>
